...
abstract contract CPATestBase is Deployers {
    ...
    function deployContracts() internal {
        ...
        CPAManager diamond = new CPAManager(
            poolManager, protocolOwner, address(cpaHook), positionManager, address(this), address(mathFacet)
        );
        cpaManager = ICPAManager(address(diamond));

        // Deploy all protocol facets (same 6 constructor args for immutables consistency)
        address[6] memory args6 = [
            address(poolManager), protocolOwner, address(cpaHook), address(positionManager), address(this), address(mathFacet)
        ];
        _registerFacets(diamond, args6);

        // Set auction manager in pool hook
        cpaHook.setAuctionManager(address(diamond));

        vm.stopPrank();
    }

    /// @notice Deploy all protocol facets and register selectors in the diamond
    function _registerFacets(CPAManager diamond, address[6] memory a) internal {
        _registerProtocolFacets(diamond, a);
        _registerCallbackFacets(diamond, a);
    }
