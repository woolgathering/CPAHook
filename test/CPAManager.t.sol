...
contract CPAManagerTest is Deployers {
	...
	function deployManager(IPoolManager _poolManager, address _owner, address _poolHook) internal returns (CPAManager) {
	...
	function deployManagerWithFallback(IPoolManager _poolManager, address _owner, address _poolHook) internal returns (CPAManager) {
		// Simple deployment since CPAManager is no longer a hook
		return new CPAManager(_poolManager, _owner, _poolHook);
	}

	function test_Constructor_Success() public {
		manager = deployManager(poolManager, owner, poolHook);
	...
	function test_Constructor_ZeroOwner() public {
		...
		manager = deployManagerWithFallback(poolManager, address(0), poolHook);
	}

	function test_Constructor_ZeroPoolManagerAndOwner() public {
		// Should revert when owner is zero address (Ownable validation)
		vm.expectRevert(); // OwnableInvalidOwner(address(0))
		manager = deployManagerWithFallback(IPoolManager(address(0)), address(0), poolHook);
	}
	...
	function test_Constructor_ValidAddresses() public {
		...
		manager = deployManager(poolManager, validOwner, poolHook);
	...
	function test_Constructor_MultipleInstances() public {
		...
		CPAManager manager1 = deployManager(poolManager, owner, poolHook);
		CPAManager manager2 = deployManager(poolManager, nonOwner, poolHook);

		// Each instance should have its own state
		assertEq(manager1.owner(), owner);
		assertEq(manager2.owner(), nonOwner);
		assertEq(manager1.cpaAuctionHookAddr(), address(poolHook));
		assertEq(manager2.cpaAuctionHookAddr(), address(poolHook));
